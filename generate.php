...
EOT;
	 ...
	 * USAGE
	 *
	 * php artisan generate:model User has_many:posts,comments has_one:phone belongs_to:city has_many_and_belongs_to:departments
	 *
	 * @param  $args array  
	 * @return void
	 */
	public function model($args)
	{
		...
		// Begin building up the file's content
		Template::new_model($class_name, 'Eloquent', $args);
		$this->prettify();
...
EOT;
	...
	public function new_model($name, $extends_class, $extra_args)
	{
		$relationships = [];
		$timestamps = 'true';

		foreach (range(1, (count($extra_args) - 1)) as $key) 
		{
			$temp = explode(':', $extra_args[$key]);

			if($temp[0] == 'timestamps')
			{
				$timestamps = $temp[1];
			}
			else
			{
				$relationships[$temp[0]] = $temp[1];
			}
		}
...
EOT;
		foreach($relationships as $method => $entity)
		{
			if(strpos($entity, ',') !== false)
			{
				foreach(explode(',', $entity) as $e)
				{
					$rel_e = ucwords($e);
					$content .= <<<EOT
